	                    ...
	                    <div class="form-group" style="width:50%;">
						  ...
						  <div class="form-check">	
							<label class="form-check-label">
	                          ...
	                          value="국내LCC항공사" name="hopse" id="hopko" class="form-check-input">국내LCC항공사
	                        </label>
						  </div>
						  <div class="form-check">	
							<label class="form-check-label">
	                          ...
	                          value="외국항공사지상직" name="hopse" id="hopos" class="form-check-input">외국항공사지상직
	                        </label>
						  </div>
						  <div class="form-check">	
							<label class="form-check-label">
	                          <input type="checkbox"
	                          <c:if test="${fn:indexOf(result.hopse, 'GHA/GSA') != -1 }">checked</c:if>
	                          value="GHA/GSA" name="hopse" id="hopstd" class="form-check-input">GHA/GSA
	                        </label>
						  </div>
						  <div class="form-check">	
							<label class="form-check-label">
	                          <input type="checkbox"
	                          <c:if test="${fn:indexOf(result.hopse, '일본공항') != -1 }">checked</c:if>
	                          value="일본항공사/공항지상직" name="hopse" id="hopgs" class="form-check-input">일본항공사/공항지상직
	                        </label>
						  </div>
						  <div class="form-check">	
							<label class="form-check-label">
	                          <input type="checkbox"
	                          <c:if test="${fn:indexOf(result.hopse, '여행사') != -1 }">checked</c:if>
	                          value="여행사취업" name="hopse" id="hopgs" class="form-check-input">여행사취업
	                        </label>
						  </div>
	                    </div>
	                    <div class="form-group" style="width:50%;">
						  ...
						  <div class="form-check">	
							<label class="form-check-label">
	                          <input type="checkbox"
	                          <c:if test="${fn:indexOf(result.worse, '06') != -1 }">checked</c:if>
	                          value="06" name="worse" id="wrlan" class="form-check-input">국비지원 자부담금
	                        </label>
						  </div>
						  <div class="form-check">	
							<label class="form-check-label">
              ...
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">이미지체크</h4>
                    ...
                    <div class="form-group">
                      <label for="input-gender">내신</label>
                      <div class="input-group">
	                    <input type="text" class="form-control input-group-append" value="${result.level }" name="level" id="level" placeholder="내신">
	                  </div>
                    </div> 
                    <div class="form-group">
                    ...
                    <table id="counsel" class="table table-striped table-bordered" style="width:100%;">
                      ...
                      <tbody>
                      	<c:forEach items="${log}" var="log">
	               			<c:choose>
	               				<c:when test="${fn:indexOf(log.path, 'crew-factory') > -1 || fn:indexOf(log.path, 'crewfac') > -1 || fn:indexOf(log.path, 'crewseoul') > -1 || fn:indexOf(log.path, 'null') > -1}">
	               					<tr>
	               						<td width="100px">${log.regdate}</td>
	               						<td width="100px">${log.ipaddr}</td>
	               						<td width="100px">${log.keyword}</td>
	               						<td width="600px">${log.path}, ${log.current}</td>
	               					</tr>
	               				</c:when>
	               				<c:otherwise>
	               					<tr>
...
<script>
	...
	function deleteRow(idx){
		if(confirm("정말로 삭제하시겠습니까?") == true){
			location.href="/manager/counsel/delete.do?reurl=/manager/counsel.do&idx="+idx;	
		}else{
			return;
		}		
	}

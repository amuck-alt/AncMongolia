	...
	<div class="row sm-row">	
		<div class="col-12 mt-4">
			<div class="col-fix-6 text-left">
				<a href="/recruit/list.do?startPage=${param.startPage}&visiblePages=${param.visiblePages}&searchText=${param.searchText}"><label class="re-label">목록으로</label></a>
			</div>
		</div>
	</div>
</div>
	...
	<div class="row sm-row">	
		...
		<div class="col-12 px-2">			
			...
			<div class="col-12 my-2 font-16 font-gray">
				...
				<c:choose>
					<c:when test="${thisDayTime <= result.stdaytime}"><span class="ml-4 tiny-label bg-pink">대기중</span></c:when>
					<c:when test="${thisDayTime >= result.enddaytime}"><span class="ml-4 tiny-label bg-navy">종료</span></c:when>
					<c:otherwise><span class="ml-4 tiny-label bg-pink">진행중</span></c:otherwise>
				</c:choose>
			</div>
		...
		<div class="col-12 px-2 pt-5 my-5">
			...
			<c:choose>
					...
					<c:otherwise><a href="${url }"><button type="button" class="btn btn-primary btn-lg btn-block font-22" style="height:4.00rem;">온라인지원하기</button></a></c:otherwise>
			</c:choose>			
		</div>
	</div>				
</div>
